...
class Group(PVGroup):
    ...
    async def get_random(self, low: int = 100, high: int = 1000) -> int:
...
class GroupDevice(ophyd.Device):
    get_random = Cpt(
        get_randomDevice, "get_random:", doc="A configurable random number"
    )
    exit = Cpt(EpicsSignal, "exit", doc="Poke me to exit")
    random1 = Cpt(
        EpicsSignal, "random1", doc="Random integer between 1 and 100"
    )
    random2 = Cpt(
        EpicsSignal,
        "random2",
