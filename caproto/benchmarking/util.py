...
class IocHandler:
    ...
    def setup_ioc(
        self, *, db_text, max_array_bytes=16384, env_vars=None, **kwargs
    ):
        if env_vars is None:
            env_vars = {}
        ...
        self.logger.debug(
            "Starting IOC with max_array_bytes=%s " "env vars: %r database: %r",
            max_array_bytes,
            env_vars,
            db_text,
        )
    ...
    for key, logger_ in logging.Logger.manager.loggerDict.items():
        if key.startswith("caproto."):
            if getattr(logger_, "level", 0) != level:
                ...
                logger.debug(
                    "Setting log level of %s to %s", key, logger_.level
                )
