...
def _convert_enum_values(
    values, to_dtype, string_encoding, enum_strings, direction
):
    ...
    if to_dtype == ChannelType.STRING:
        ...
        def get_value(v):
            ...
            raise ConversionError(
                f"Invalid enum index: {v!r} " f"count={num_strings}"
            )
    ...
    if to_dtype == ChannelType.STRING:
        if direction == ConversionDirection.TO_WIRE:
            ...
            if isinstance(values, str):
                return [
                    bytes([v]) for v in encode_or_fail(values, string_encoding)
                ]
...
def _convert_string_values(
    ...
    if (
        ...
        or to_dtype == ChannelType.ENUM
    ):
        # from the wire (or for enums), decode bytes -> strings
        values = [
            decode_or_fail(v, string_encoding) if isinstance(v, bytes) else v
            for v in values
        ]
    ...
    if to_dtype == ChannelType.ENUM:
        ...
        return _convert_enum_values(
            values,
            to_dtype=ChannelType.INT,
    ...
    *,
    ...
    if from_dtype in (ChannelType.STSACK_STRING, ChannelType.CLASS_NAME) or (
        ...
        if from_dtype != to_dtype:
            raise ConversionError(
                "Cannot convert values for stsack_string or "
    ...
    return backend.python_to_epics(
        to_dtype, values, byteswap=byteswap, convert_from=from_dtype
    )
...
class ChannelAlarm:
    ...
    status = _read_only_property("status", doc="Current alarm status")
    severity = _read_only_property("severity", doc="Current alarm severity")
    ...
    alarm_string = _read_only_property(
        "alarm_string", doc="String associated with alarm"
    )
    ...
    async def write(
        ...
        *,
        ...
        if severity is not None:
            ...
            if (
                not self.must_acknowledge_transient
                or self.severity_to_acknowledge < self.severity
            ):
        ...
        if must_acknowledge_transient is not None:
            ...
            if (
                not must_acknowledge_transient
                and self.severity_to_acknowledge > self.severity
            ):
...
class ChannelData:
    ...
    def __init__(
        ...
        *,
        ...
        self._data = dict(value=value, timestamp=timestamp)
    ...
    async def auth_read(
        self, hostname, username, data_type, *, user_address=None
    ):
        ...
        if AccessRights.READ not in access:
            raise Forbidden(
                "Client with hostname {} and username {} cannot "
    ...
    async def write_from_dbr(self, data, data_type, metadata):
        ...
        try:
            ...
            await self.alarm.write(
                status=AlarmStatus.WRITE, severity=AlarmSeverity.MAJOR_ALARM
            )
            raise

        self._data["value"] = (
            modified_value if modified_value is not None else value
        )
        ...
        if metadata is None:
            ...
            metadata_dict = dbr_metadata_to_dict(
                dbr_metadata, self.string_encoding
            )
    ...
    def _read_metadata(self, dbr_metadata):
        ...
        if hasattr(dbr_metadata, "units"):
            ...
            if isinstance(units, str):
                units = units.encode(
                    self.string_encoding if self.string_encoding else "latin-1"
                )
    ...
    async def write_metadata(
        ...
        if any(alarm_val is not None for alarm_val in (status, severity)):
...
class ChannelEnum(ChannelData):
    ...
    def _read_metadata(self, dbr_metadata):
        if isinstance(
            dbr_metadata,
            (DBR_TYPES[ChannelType.GR_ENUM], DBR_TYPES[ChannelType.CTRL_ENUM]),
        ):
            dbr_metadata.enum_strings = [
                s.encode(self.string_encoding) for s in self.enum_strings
            ]
...
class ChannelByte(ChannelNumeric):
    ...
    def __init__(
        self, *, value=None, max_length=100, string_encoding=None, **kwargs
    ):
...
class ChannelChar(ChannelData):
    ...
    def __init__(
        self, *, value=None, max_length=100, string_encoding="latin-1", **kwargs
    ):
