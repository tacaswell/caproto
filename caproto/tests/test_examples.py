...
def test_thread_client_example(curio_server):
    ...
    def client():
        example_main(pvname1=prefix + "pi", pvname2=prefix + "str")
...
def test_curio_server_example(prefix):
    ...
    async def run_client():
        ...
        await chan1.write(
            (7,), data_type=ca.ChannelType.TIME_DOUBLE, metadata=metadata
        )
        ...
        metadata = (severity + 1,)
        await chan1.write(
            (), data_type=ca.ChannelType.PUT_ACKS, metadata=metadata
        )
        ...
        metadata = (1,)
        await chan1.write(
            (), data_type=ca.ChannelType.PUT_ACKT, metadata=metadata
        )
...
def _test_ioc_examples(
    request, module_name, pvdb_class_name, class_kwargs, prefix
):
    ...
    def find_put_value(pv):
        ...
        for put_class, put_value in put_values:
            ...
            raise Exception(
                "Failed to set default value for channeldata:"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "caproto.ioc_examples.currency_conversion",
            "CurrencyConversionIOC",
            {},
        ),
    ],
)
def test_flaky_ioc_examples(
    request, module_name, pvdb_class_name, class_kwargs, prefix
):
    return _test_ioc_examples(
        request, module_name, pvdb_class_name, class_kwargs, prefix
