...
def test_nonet():
    ...
    # one datagram.
    bytes_to_send = cli_b.send(
        ca.VersionRequest(0, 13), ca.SearchRequest(pv1, 0, 13)
    )
    ...
    bytes_to_send = srv_b.send(
        ...
        ca.SearchResponse(5064, None, search_req.cid, 13),
    )
    ...
    circuit = ca.VirtualCircuit(our_role=ca.CLIENT, address=address, priority=0)
    ...
    srv_circuit = ca.VirtualCircuit(
        our_role=ca.SERVER, address=address, priority=None
    )
    ...
    cli_send(
        chan1.circuit, ca.CreateChanRequest(name=pv1, cid=chan1.cid, version=13)
    )
    ...
    srv_send(
        srv_circuit,
        ca.CreateChanResponse(cid=chan1.cid, sid=1, data_type=5, data_count=1),
    )
    ...
    cancel_req = ca.EventCancelRequest(
        data_type=add_req.data_type,
        sid=add_req.sid,
    ...
    cli_send(
        chan1.circuit,
        ca.ReadNotifyRequest(data_type=5, data_count=1, sid=chan1.sid, ioid=12),
    )
    srv_recv(srv_circuit)
    srv_send(
        srv_circuit,
        ca.ReadNotifyResponse(
            data=(3,), data_type=5, data_count=1, ioid=12, status=1
        ),
    )
    cli_recv(chan1.circuit)

    # Test writing.
    request = ca.WriteNotifyRequest(
        data_type=2, data_count=1, sid=chan1.sid, ioid=13, data=(4,)
    )
    ...
    srv_send(
        srv_circuit,
        ca.WriteNotifyResponse(data_type=5, data_count=1, ioid=13, status=1),
    )
    cli_recv(chan1.circuit)

    # Test "clearing" (closing) the channel.
    cli_send(
        chan1.circuit, ca.ClearChannelRequest(sid=chan1.sid, cid=chan1.cid)
    )
