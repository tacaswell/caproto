...
def pytest_bench_machine_to_asv(
    root, *, node, machine, system, release, cpu, **kw
):
...
def asv_bench_result(name, md_list):
    ...
    md0 = md_list[0]
    if len(md_list) == 1:
        return single_asv_bench_result(
            start_dt=md0["start_dt"],
            end_dt=md0["end_dt"],
            **md0["pytest_result"]
        )
    else:
        ...
        def get_stat_key(key):
            return [
                res["stats"][key] if res and "stats" in res else None
...
def pytest_bench_outline_to_asv():
    ...
    outlined = {
        name: asv_bench_outline(name, md_list)
        for name, md_list in asv_metadata.items()
    }
...
def pytest_bench_to_asv(root):
    ...
    commit_dt = datetime.strptime(commit_info["time"][:-6], "%Y-%m-%dT%H:%M:%S")
...
def save_asv_results(asv_path, *, machine_info, bench_outline, bench_results):
    ...
    results_fn = "{}-{}.json".format(
        bench_results["commit_hash"][:8], bench_results["env_name"]
    )
...
def find_asv_root(
    path, *, asv_conf_filename="asv.conf.json", asv_subdir=".asv"
):
...
def pytest_benchmark_update_json(config, benchmarks, output_json):
    ...
    try:
        ...
        logger.exception(
            "Failed to update pytest benchmark json; " "asv results not saved"
        )
...
class AsvBenchmarkFixture(BenchmarkFixture):
    ...
    def __call__(self, function_to_benchmark, *args, **kwargs):
        ...
        try:
            ...
            md = self.generate_asv_metadata(
                function_to_benchmark, start_dt, end_dt
            )
            name = md["name"]
            if name not in asv_metadata:
                asv_metadata[name] = []

            param_names, param_values = get_all_params_from_marked_test(
                self.node
            )
...
def asv_bench(request):
    ...
    if bs.skip:
        pytest.skip(
            "[asv] Benchmarks are skipped (--benchmark-skip was " "used)."
        )
    else:
        node = request.node
...
def get_conftest_globals():
    ...
    return dict(
        pytest_benchmark_update_json=pytest_benchmark_update_json,
        benchmark=asv_bench,
    )
