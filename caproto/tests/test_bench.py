...
def setup_module(module):
    ...
    many_connections_db = {
        ("connections:{}".format(i), "ao"): dict(VAL=i) for i in range(1000)
    }
...
def test_waveform_get(benchmark, waveform_size, backend, log_level):
    ...
    context = {
        "pyepics": bench_pyepics_get_speed,
...
def test_waveform_put(benchmark, waveform_size, backend, log_level):
    ...
    context = {
        "pyepics": bench_pyepics_put_speed,
...
@contextlib.contextmanager
def bench_pyepics_many_connections(
    pv_names, *, initial_value=None, log_level="DEBUG"
):
    import epics
    ...
    def pyepics():
        ...
        pvs = [epics.PV(pvname, auto_monitor=False) for pvname in pv_names]
...
@contextlib.contextmanager
def bench_threading_many_connections(
    pv_names, *, initial_value=None, log_level="DEBUG"
):
    shared_broadcaster, context, PV = _setup_threading_client(log_level)
    pvs = []
...
@contextlib.contextmanager
def bench_curio_many_connections(
    pv_names, *, initial_value=None, log_level="DEBUG"
):

    async def test():
        ctx = await get_curio_context(log_level=log_level)
        channels = await ctx.create_many_channels(
            *pv_names, wait_for_connection=True, move_on_after=20
        )
...
@contextlib.contextmanager
def bench_trio_many_connections(
    pv_names, *, initial_value=None, log_level="DEBUG"
):
    ...
    async def test(context):
        ...
        channels = await context.create_many_channels(
            *pv_names, wait_for_connection=True, move_on_after=10
        )
...
@pytest.mark.parametrize("log_level", ["INFO"])
def test_many_connections_same_ioc(
    benchmark, backend, connection_count, pv_format, log_level
):
    ca.benchmarking.set_logging_level(logging.DEBUG, logger=logger)

    context = {
        "pyepics": bench_pyepics_many_connections,
