...
def run_example_ioc(
    ...
    *,
    ...
    p = subprocess.Popen(
        [sys.executable, "-m", "caproto.tests.example_runner", module_name]
        + list(args),
        stdout=stdout,
        stderr=stderr,
        stdin=stdin,
...
def poll_readiness(pv_to_check, attempts=15):
    ...
    for attempt in range(attempts):
        ...
        raise TimeoutError(
            f"ioc fixture failed to start in "
...
def run_softioc(request, db, additional_db=None, **kwargs):
    ...
    ioc_handler.setup_ioc(db_text=db_text, max_array_bytes="10000000", **kwargs)
...
def epics_base_ioc(prefix, request):
    ...
    db = {
        ("{}waveform".format(prefix), "waveform"): dict(FTVL="LONG", NELM=4000),
        ("{}float".format(prefix), "ai"): dict(VAL=1),
        ("{}enum".format(prefix), "bi"): dict(VAL=1, ZNAM="zero", ONAM="one"),
...
***********************************""",
    ...
    pvs = {pv[len(prefix) :]: pv for pv, rtype in db}

    return SimpleNamespace(
        process=process, prefix=prefix, name=name, pvs=pvs, type="epics-base"
    )
...
def start_repeater():
    ...
    _repeater_process = run_example_ioc(
        "--script",
...
def pvdb_from_server_example():
    ...
    pvdb = {
        ...
        "enum": ca.ChannelEnum(value="b", enum_strings=["a", "b", "c", "d"]),
        "enum2": ca.ChannelEnum(
            value="bb", enum_strings=["aa", "bb", "cc", "dd"]
        ),
        "int": ca.ChannelInteger(value=96, units="doodles"),
        "char": ca.ChannelByte(
            value=b"3",
            units="poodles",
            lower_disp_limit=33,
        ...
        "str": ca.ChannelString(
            value="hello", string_encoding="latin-1", alarm=alarm
        ),
        "str2": ca.ChannelString(
            value="hello", string_encoding="latin-1", alarm=alarm
        ),
        "stra": ca.ChannelString(
            value=["hello", "how is it", "going"], string_encoding="latin-1"
        ),
    }
...
def curio_server(prefix):
    ...
    async def _server(pvdb):
        ...
        ctx = caproto.curio.server.Context(
            SERVER_HOST, port, pvdb, log_level="DEBUG"
        )
        try:
            await ctx.run()
...
def run_with_trio_context(func, *, log_level="DEBUG", **kwargs):
    ...
    async def runner():
        async with trio.open_nursery() as nursery:
            ...
            await broadcaster.register()
            logger.debug("Registered! Returning new context.")
            context = caproto.trio.client.Context(
                broadcaster, nursery=nursery, log_level=log_level
            )
...
@pytest.fixture(scope="function", params=["curio", "trio"])
def server(request):
