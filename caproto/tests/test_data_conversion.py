...
def run_conversion_test(
    ...
    *,
    ...
    def _test():
        print(f"--- {direction} ---")
        print(
            f"Convert: {from_dtype.name} {values!r} -> {to_dtype.name} "
    ...
    if direction == FROM_WIRE:
        ...
        values = backend.epics_to_python(
            values, from_dtype, data_count=count, auto_byteswap=True
        )
    ...
    if isclass(expected) and issubclass(expected, Exception):
        ...
        if isinstance(returned, array.array):
            ...
            print(
                f"array to list {returned} -> {returned.tolist()}"
                f" ({returned.typecode})"
            )
            ...
            print(
                f"numpy to list {returned} -> {returned.tolist()}"
                f" ({returned.dtype})"
            )
            returned = returned.tolist()
...
enum_strs = dict(enum_strings=["aa", "bb", "cc"], string_encoding="ascii")
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", string_to_wire_tests
)
def test_string_to_wire(
    backends, from_dtype, to_dtype, values, expected, kwargs
):
    run_conversion_test(
        values=values,
        from_dtype=from_dtype,
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", string_decoding_tests
)
def test_string_from_wire(
    backends, from_dtype, to_dtype, values, expected, kwargs
):
    run_conversion_test(
        values=values,
...
ascii_encoding = dict(
    string_encoding="ascii", enum_strings=["abc", "def", "ghi"]
)
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", string_array_to_wire_tests
)
def test_string_array_to_wire(
    backends, from_dtype, to_dtype, values, expected, kwargs
):
    run_conversion_test(
        values=values,
...
def no_encoding(count):
    return dict(
        string_encoding=None, count=count, enum_strings=["abc", "def", "ghi"]
    )


def ascii_encoding(count):
    return dict(
        string_encoding="ascii", count=count, enum_strings=["abc", "def", "ghi"]
    )
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", string_array_from_wire
)
def test_string_array_from_wire(
    backends, from_dtype, to_dtype, values, expected, kwargs
):
    run_conversion_test(
        values=values,
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", char_to_wire
)
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", char_from_wire_tests
)
def test_char_from_wire(
    backends, from_dtype, to_dtype, values, expected, kwargs
):
    run_conversion_test(
        values=values,
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", enum_to_wire_tests
)
...
@pytest.mark.parametrize(
    "from_dtype, to_dtype, values, expected, kwargs", long_to_wire
)
def test_long_to_wire(backends, from_dtype, to_dtype, values, expected, kwargs):
    run_conversion_test(
        values=values,
        from_dtype=from_dtype,
        to_dtype=to_dtype,
