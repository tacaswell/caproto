...
def _link_parent_attribute(
    pvprop, parent_attr_name, *, read_only=False, use_setattr=False, default=0
):
...
class AiFields(RecordFieldGroup):
    ...
    initialized = pvproperty(
        name="INIT", dtype=ChannelType.LONG, doc="Initialized?", read_only=True
    )
...
records = {
    record._record_type: record
    for name, record in globals().items()
    if inspect.isclass(record)
    and issubclass(record, PVGroup)
    and record not in (PVGroup, RecordFieldGroup)
}
