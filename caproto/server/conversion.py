...
def pvfunction_to_device_function(name, pvf, *, indent="    "):
    ...
    args = ", ".join(
        format_arg(spec) for spec in pvf.pvspec if spec.attr not in skip_attrs
    )
    ...
    for pvspec in pvf.pvspec:
        if pvspec.attr not in skip_attrs:
            yield (
                f"{indent*2}self.{pvspec.attr}.put({pvspec.attr}, " "wait=True)"
            )
...
def group_to_device(group):
    ...
    for name, subgroup in group._subgroups_.items():
        ...
        yield from _format(
            f"{name.lower()} = Cpt({name}Device, " f"'{subgroup.prefix}'{doc})",
            indent=4,
        )
    ...
    for name, prop in group._pvs_.items():
        ...
        yield from _format(
            f"{name.lower()} = Cpt({cls}, '{pvspec.name}'" f"{string}{doc})",
            indent=4,
        )
...
def record_to_field_info(record_type):
    ...
    for name, field_info in field_dict.items():
        ...
        if type_ == "DBF_MENU":
            ...
            kwargs["enum_strings"] = (
                f"menus.{field_info.menu}" ".get_string_tuple()"
            )
...
def record_to_high_level_group_code(record_type, *, skip_fields=None):
    ...
    for name, cls, kwargs, finfo in record_to_field_info(record_type):
        ...
        yield from _format(
            f"{name} = pvproperty(name='{finfo.field}', "
