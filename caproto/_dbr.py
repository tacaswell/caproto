...
class DbrStringArray(collections.UserList):
    ...
    def tobytes(self):
        # numpy compat
        return b"".join(
            item[:MAX_STRING_SIZE].ljust(MAX_STRING_SIZE, b"\x00")
            for item in self
        )
...
class DbrTypeBase(ctypes.BigEndianStructure):
    ...
    def to_dict(self):
        d = {field: getattr(self, field) for field in self.info_fields}
        if "status" in d:
    ...
    def __repr__(self):
        formatted_args = ", ".join(
            ["{!s}={!r}".format(k, v) for k, v in self.to_dict().items()]
        )
...
class TimeStamp(DbrTypeBase):
    ...
    def timestamp(self):
        ...
        return epics_timestamp_to_unix(self.secondsSinceEpoch, self.nanoSeconds)
...
class ControlTypeUnits(GraphicControlUnits):
    """DBR_CTRL with units"""
    info_fields = (
        GraphicControlBase.info_fields
...
class GraphicTypePrecision(GraphicControlPrecision):
    ...
    info_fields = GraphicControlBase.info_fields + ("precision", "units")
...
class _EnumWithStrings:
    ...
    def enum_strings(self):
        ...
        return tuple(self.strs[i].value for i in range(self.no_str))
...
special_types = {
    ChannelType.PUT_ACKT,
    ChannelType.PUT_ACKS,
    ChannelType.STSACK_STRING,
...
graphical_types = (
    set(field_types["graphic"].values())
    - set(special_types)
    - {ChannelType.STS_STRING}
)
control_types = (
    set(field_types["control"].values())
    - set(special_types)
...
char_types = {
    ChannelType.CHAR,
    ChannelType.TIME_CHAR,
    ChannelType.CTRL_CHAR,
...
int_types = {
    ...
    ChannelType.LONG,
    ChannelType.TIME_LONG,
    ChannelType.CTRL_LONG,
}
...
DBR_TYPES = {
    ...
    if (
        name.startswith("DBR_")
        and issubclass(cls, DbrTypeBase)
