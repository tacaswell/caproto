...
CAStatusCode = namedtuple(
    "CAStatusCode",
    "name code code_with_severity severity success " "defunct description",
)
...
ECA_TOLARGE = _ca_status_code(
    ...
    code=9,
    desc=(
        "The requested data transfer is greater than available memory "
...
ECA_DISCONNCHID = _ca_status_code(
    ...
    code=13,
    desc=(
        "The request was ignored because the specified channel is "
        "disconnected"
    ),
    defunct=True,
)
...
ECA_BADSTR = _ca_status_code(
    "ECA_BADSTR", severity=CASeverity.ERROR, code=23, desc="Invalid string"
)
...
ECA_NEEDSFP = _ca_status_code(
    ...
    code=28,
    desc=(
        "Unable to initialize without the vxWorks VX_FP_TASKtask " "option set"
    ),
    defunct=True,
)
...
ECA_NOTTHREADED = _ca_status_code(
    ...
    code=57,
    desc=(
        "Preemptive callback not enabled - additional threads may not "
        "join context"
    ),
)
ECA_16KARRAYCLIENT = _ca_status_code(
    ...
    code=58,
    desc=(
        "Client’s protocol revision does not support transfers "
        "exceeding 16k bytes"
    ),
)
...
eca_value_to_status = {
    value.code_with_severity: value
